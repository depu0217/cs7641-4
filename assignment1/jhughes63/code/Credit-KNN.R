# This is the Credit-KNN.R file. It runs the credit.csv file through the KNN Supervised Learning algorithm
# Initial conditions:
#     All packages must be installed so library calls below will not give errors
#
# Citations;
...
for (i in seq(1:5)){
  ...
  creditTrain <- credit[1:(split),2:20]
  creditTest <- credit[(testBase+1):nrow(credit),2:20]
  creditTrainLabels <- credit[1:split, 1]
  creditTestLabels <- credit[(testBase+1):nrow(credit), 1]
  
  # Test Run #1; Train, predict, check results; k=5
  startTime<-as.POSIXlt(Sys.time(), "UTC")
  pred <- knn(train=creditTrain, test=creditTest, cl=creditTrainLabels, k=11)
  print("Test Time: ")
